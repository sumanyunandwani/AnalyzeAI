    ...
    async def create_schema_if_not_exists(self):
        ...
        try:
            async with self._engine.begin() as conn:
                await conn.execute(select(Businesses))
        except ProgrammingError:
            # If the table does not exist, create all tables
            async with self._engine.begin() as conn:
                await conn.run_sync(Base.metadata.create_all)

    async def seed_business_domains(self) -> bool:
        """
        Seeds the 'businesses' table with initial business domains if it's empty.
        """
        # Predefined business domains
        business_domains = [
            "Technology",
            "Healthcare",
            "Finance",
        ...
        async with self.get_session() as session:
            for domain in business_domains:
                existing = await session.execute(
                    select(Businesses).where(Businesses.name == domain)
                    )
                if not existing.scalars().first():
                    new_business = Businesses(name=domain)
